...
void MipiRgb::common_setup_() {
  ...
  if (err == ESP_OK)
    err = esp_lcd_panel_disp_on_off(this->handle_, true);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "lcd setup failed: %s", esp_err_to_name(err));
    this->mark_failed(LOG_STR("lcd setup failed"));
  }
  ESP_LOGCONFIG(TAG, "MipiRgb setup complete using Waveshare RGB panel settings");
}
...
void MipiRgb::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  RGB Panel Setup: Waveshare override");
